...
const PreviewGroupImage = ({navigation, route}) => {
    ...
    const updateFoto = async() => {
        ...
        firestore().collection('groupChat').doc(route.params.chatID).collection("pesanGrup")
            .add({
                waktuPesan: firebase.firestore.FieldValue.serverTimestamp(),
                isiPesan: input,
                urlGambar: imgUrl,
                idPengirim: route.params.idPengirim,
                namaPengirim: route.params.namaPengirim,
                fotoPengirim: route.params.fotoPengirim,
                tipePesan: "gambar"
            })
        .then(
            navigation.goBack
